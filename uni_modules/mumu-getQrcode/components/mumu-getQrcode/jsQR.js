	...
	([
			...
			var GenericGF = /** @class */ (function() {
				function GenericGF(primitive, size, genBase) {
					...
					for (var i = 0; i < this.size; i++) {
						...
						x = x * 2;
						if (x >= this.size) {
							x = (x ^ this.primitive) & (this.size -
							1); // tslint:disable-line:no-bitwise
						}
					}
				...
				GenericGF.prototype.multiply = function(a, b) {
					...
					return this.expTable[(this.logTable[a] + this.logTable[b]) % (this.size -
						1)];
				};
			...
			var GenericGFPoly = /** @class */ (function() {
				function GenericGFPoly(field, coefficients) {
					...
					if (coefficientsLength > 1 && coefficients[0] === 0) {
						...
						if (firstNonZero === coefficientsLength) {
							...
							this.coefficients = new Uint8ClampedArray(coefficientsLength -
								firstNonZero);
				...
				GenericGFPoly.prototype.addOrSubtract = function(other) {
					...
					if (smallerCoefficients.length > largerCoefficients.length) {
						_a = [largerCoefficients, smallerCoefficients], smallerCoefficients =
							_a[0], largerCoefficients = _a[1];
					}
					...
					for (var i = lengthDiff; i < largerCoefficients.length; i++) {
						sumDiff[i] = GenericGF_1.addOrSubtractGF(smallerCoefficients[i -
				...
				GenericGFPoly.prototype.multiplyPoly = function(other) {
					...
					for (var i = 0; i < aLength; i++) {
						...
						for (var j = 0; j < bLength; j++) {
							product[i + j] = GenericGF_1.addOrSubtractGF(product[i + j], this
				...
				GenericGFPoly.prototype.evaluateAt = function(a) {
					...
					for (var i = 1; i < size; i++) {
						result = GenericGF_1.addOrSubtractGF(this.field.multiply(a, result),
							this.coefficients[i]);
					}
					return result;
				};
		...
		(function(module, exports, __webpack_require__) {
					...
					if (decoded) {
						return {
							...
							location: {
								...
								bottomRightCorner: extracted.mappingFunction(location_1.dimension,
			...
			function jsQR(data, width, height, providedOptions) {
				...
				var shouldInvert = options.inversionAttempts === "attemptBoth" || options
					.inversionAttempts === "invertFirst";
				var tryInvertedFirst = options.inversionAttempts === "onlyInvert" || options
					.inversionAttempts === "invertFirst";
				var _a = binarizer_1.binarize(data, width, height, shouldInvert),
					binarized = _a.binarized,
					inverted = _a.inverted;
				var result = scan(tryInvertedFirst ? inverted : binarized);
				if (!result && (options.inversionAttempts === "attemptBoth" || options.inversionAttempts ===
						"invertFirst")) {
		...
		(function(module, exports, __webpack_require__) {
			...
			function binarize(data, width, height, returnInverted) {
				...
				for (var verticalRegion = 0; verticalRegion < verticalRegionCount; verticalRegion++) {
					for (var hortizontalRegion = 0; hortizontalRegion <
						horizontalRegionCount; hortizontalRegion++) {
						var sum = 0;
						var min = Infinity;
						var max = 0;
						for (var y = 0; y < REGION_SIZE; y++) {
							for (var x = 0; x < REGION_SIZE; x++) {
								var pixelLumosity = greyscalePixels.get(hortizontalRegion * REGION_SIZE + x,
						...
						if (max - min <= MIN_DYNAMIC_RANGE) {
							...
							if (verticalRegion > 0 && hortizontalRegion > 0) {
								...
								var averageNeighborBlackPoint = (blackPoints.get(hortizontalRegion,
										verticalRegion - 1) +
		...
		(function(module, exports, __webpack_require__) {
			...
			function buildFunctionPatternMask(version) {
				...
				for (var _i = 0, _a = version.alignmentPatternCenters; _i < _a.length; _i++) {
					...
					for (var _b = 0, _c = version.alignmentPatternCenters; _b < _c.length; _b++) {
						var y = _c[_b];
						if (!(x === 6 && y === 6 || x === 6 && y === dimension - 7 || x === dimension - 7 &&
								y === 6)) {
			...
			function readVersion(matrix) {
				...
				for (var _i = 0, VERSIONS_1 = version_1.VERSIONS; _i < VERSIONS_1.length; _i++) {
					...
					if (version.infoBits === topRightVersionBits || version.infoBits ===
			...
			function readFormatInformation(matrix) {
				...
				for (var y = dimension - 1; y >= dimension - 7; y--) { // bottom left
					topRightBottomRightFormatInfoBits = pushBit(matrix.get(8, y),
						topRightBottomRightFormatInfoBits);
				}
				for (var x = dimension - 8; x < dimension; x++) { // top right
					topRightBottomRightFormatInfoBits = pushBit(matrix.get(x, 8),
						topRightBottomRightFormatInfoBits);
				}
				var bestDifference = Infinity;
				var bestFormatInfo = null;
				for (var _i = 0, FORMAT_INFO_TABLE_1 = FORMAT_INFO_TABLE; _i < FORMAT_INFO_TABLE_1
					.length; _i++) {
					...
					if (topLeftFormatInfoBits !==
						topRightBottomRightFormatInfoBits) { // also try the other option
			...
			function decodeMatrix(matrix) {
				...
				for (var _i = 0, dataBlocks_3 = dataBlocks; _i < dataBlocks_3.length; _i++) {
					...
					var correctedBytes = reedsolomon_1.decode(dataBlock.codewords, dataBlock.codewords
		...
		(function(module, exports, __webpack_require__) {
			...
			function decodeAlphanumeric(stream, size) {
				...
				while (length >= 2) {
					...
					var b = v % 45;
					bytes.push(AlphanumericCharacterCodes[a].charCodeAt(0), AlphanumericCharacterCodes[b]
						.charCodeAt(0));
				...
				BitStream.prototype.readBits = function(numBits) {
					...
					if (numBits > 0) {
						...
						if (numBits > 0) {
							...
							result = (result << numBits) | ((this.bytes[this.byteOffset] &
								mask) >> bitsToNotRead);
		...
		(function(module, exports, __webpack_require__) {
			...
			function findErrorMagnitudes(field, errorEvaluator, errorLocations) {
				...
				for (var i = 0; i < s; i++) {
					...
					for (var j = 0; j < s; j++) {
						if (i !== j) {
							denominator = field.multiply(denominator, GenericGF_1.addOrSubtractGF(1, field
								.multiply(errorLocations[j], xiInverse)));
						}
					}
					result[i] = field.multiply(errorEvaluator.evaluateAt(xiInverse), field.inverse(
						denominator));
			...
			function decode(bytes, twoS) {
				...
				for (var i = 0; i < errorLocations.length; i++) {
					...
					outputBytes[position] = GenericGF_1.addOrSubtractGF(outputBytes[position],
						errorMagnitudes[i]);
				}
				return outputBytes;
			}
		...
		(function(module, exports, __webpack_require__) {
			...
			function extract(image, location) {
				...
				var sToQ = squareToQuadrilateral(location.topLeft, location.topRight, location
					.alignmentPattern, location.bottomLeft);
		...
		(function(module, exports, __webpack_require__) {
			...
			function reorderFinderPatterns(pattern1, pattern2, pattern3) {
				...
				if (twoThreeDistance >= oneTwoDistance && twoThreeDistance >= oneThreeDistance) {
					_a = [pattern2, pattern1, pattern3], bottomLeft = _a[0], topLeft = _a[1], topRight = _a[
						2];
				} else if (oneThreeDistance >= twoThreeDistance && oneThreeDistance >= oneTwoDistance) {
					_b = [pattern1, pattern2, pattern3], bottomLeft = _b[0], topLeft = _b[1], topRight = _b[
						2];
				} else {
					_c = [pattern1, pattern3, pattern2], bottomLeft = _c[0], topLeft = _c[1], topRight = _c[
						2];
				}
				// Use cross product to figure out whether bottomLeft (A) and topRight (C) are correct or flipped in relation to topLeft (B)
				// This asks whether BC x BA has a positive z component, which is the arrangement we want. If it's negative, then
				// we've got it flipped around and should swap topRight and bottomLeft.
				if (((topRight.x - topLeft.x) * (bottomLeft.y - topLeft.y)) - ((topRight.y - topLeft.y) * (
						bottomLeft.x - topLeft.x)) < 0) {
			...
			function countBlackWhiteRun(origin, end, matrix, length) {
				...
				var middleValue = towardsEnd.shift() + awayFromEnd.shift() -
			...
			function scorePattern(point, ratios, matrix) {
				try {
					...
					var topLeftBottomRightRun = countBlackWhiteRun(point, topLeftPoint, matrix, ratios
						.length);
					var bottomLeftPoint = {
						x: Math.min(matrix.width, point.x + point.y) + 1,
						y: Math.min(matrix.height, point.y + point.x) + 1,
					};
					var bottomLeftTopRightRun = countBlackWhiteRun(point, bottomLeftPoint, matrix, ratios
						.length);
					...
					var avgSize = (horzError.averageSize + vertError.averageSize + diagDownError
						.averageSize + diagUpError.averageSize) / 4;
			...
			function locate(matrix) {
				...
				var _loop_1 = function(y) {
					...
					var _loop_2 = function(x) {
						...
						if (v === lastBit) {
							...
							var validFinderPattern = Math.abs(scans[0] -
									averageFinderPatternBlocksize) <
								averageFinderPatternBlocksize &&
								Math.abs(scans[1] - averageFinderPatternBlocksize) <
								averageFinderPatternBlocksize &&
								Math.abs(scans[2] - 3 * averageFinderPatternBlocksize) < 3 *
								averageFinderPatternBlocksize &&
							...
							var validAlignmentPattern = Math.abs(scans[2] -
							...
							if (validFinderPattern) {
								...
								var matchingQuads = activeFinderPatternQuads.filter(function(q) {
									return (startX_1 >= q.bottom.startX && startX_1 <= q
											.bottom.endX) ||
										(endX_1 >= q.bottom.startX && startX_1 <= q.bottom
											.endX) ||
										(startX_1 <= q.bottom.startX && endX_1 >= q.bottom
											.endX && ((scans[2] / (q.bottom.endX - q.bottom
													.startX)) < MAX_QUAD_RATIO &&
												(scans[2] / (q.bottom.endX - q.bottom
													.startX)) > MIN_QUAD_RATIO));
								});
							...
							if (validAlignmentPattern) {
								...
								var matchingQuads = activeAlignmentPatternQuads.filter(function(q) {
									return (startX_2 >= q.bottom.startX && startX_2 <= q
											.bottom.endX) ||
										(endX_2 >= q.bottom.startX && startX_2 <= q.bottom
											.endX) ||
										(startX_2 <= q.bottom.startX && endX_2 >= q.bottom
											.endX && ((scans[2] / (q.bottom.endX - q.bottom
													.startX)) < MAX_QUAD_RATIO &&
												(scans[2] / (q.bottom.endX - q.bottom
													.startX)) > MIN_QUAD_RATIO));
					...
					finderPatternQuads.push.apply(finderPatternQuads, activeFinderPatternQuads.filter(
						function(q) {
				...
				finderPatternQuads.push.apply(finderPatternQuads, activeFinderPatternQuads.filter(function(
					q) {
				...
				var finderPatternGroups = finderPatternQuads
					...
					.map(function(q) {
						...
						var lengths = [q.top.endX - q.top.startX, q.bottom.endX - q.bottom.startX, q
							.bottom.y - q.top.y + 1
						];
						...
						var otherPoints = finderPatterns
							...
							.map(function(p) {
								return ({
									x: p.x,
									y: p.y,
									score: p.score + (Math.pow((p.size - point.size), 2)) /
										point.size,
									size: p.size
								});
							})
				...
				var _a = reorderFinderPatterns(finderPatternGroups[0].points[0], finderPatternGroups[0]
						.points[1], finderPatternGroups[0].points[2]),
					topRight = _a.topRight,
					topLeft = _a.topLeft,
					bottomLeft = _a.bottomLeft;
				var alignment = findAlignmentPattern(matrix, alignmentPatternQuads, topRight, topLeft,
					bottomLeft);
				var result = [];
				if (alignment) {
					result.push({
				...
				var centeredAlignment = findAlignmentPattern(matrix, alignmentPatternQuads, midTopRight,
					midTopLeft, midBottomLeft);
			...
			function findAlignmentPattern(matrix, alignmentPatternQuads, topRight, topLeft, bottomLeft) {
				...
				var moduleSize;
				try {
					(_a = computeDimension(topLeft, topRight, bottomLeft, matrix), dimension = _a.dimension,
				...
				var modulesBetweenFinderPatterns = ((distance(topLeft, bottomLeft) + distance(topLeft,
					topRight)) / 2 / moduleSize);
				...
				var alignmentPattern = modulesBetweenFinderPatterns >= 15 && alignmentPatterns.length ?
					alignmentPatterns[0] : expectedAlignmentPattern;
				return {
